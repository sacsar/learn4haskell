...
subList i j l
    ...
    | otherwise =
        let numElts = j - i + 1
            withoutPrefix = drop i l
        in take numElts withoutPrefix
...
takeEven l = go [] l
    where
        ...
        go acc [] = acc
        go acc (x:[]) = acc ++ [x]
        go acc (x:_:xs) = acc ++ [x] ++ takeEven xs -- this feels weird
...
-}
-- how are you meant to use map and not flatMap???
...
-}
contains :: (Eq a) => a -> [[a]] -> [[a]]
contains x ls = filter (elem x) ls
...
rotate n l
    | n < 0 = []
    | length l == 0 = l
    | otherwise = take ll (drop n looped)
        where looped = cycle l
              ll = length l
